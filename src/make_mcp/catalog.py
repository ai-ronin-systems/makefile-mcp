...
from make_mcp.errors import ContextNotFound, MakeInspectionError, TaskNotExposed, TaskNotFound
from make_mcp.filesystem import Fingerprint, ensure_within_root, fingerprint
from make_mcp.makefile import StaticMakeInspector
from make_mcp.models import MakeMcpConfig, ProjectContext, TaskDefinition, TaskRisk
    ...
    def snapshot(self, context_name: str = "root") -> CatalogSnapshot:
        ...
        for name, target in raw.targets.items():
            if not self.governed:
                # Auto mode mirrors lightweight Make MCP servers: every target our conservative
