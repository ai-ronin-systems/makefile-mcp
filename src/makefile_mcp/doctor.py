...
from makefile_mcp.errors import MakefileMcpError
...
def _effective_path(config: MakefileMcpConfig) -> str:
...
def _runtime_findings(root: Path, config: MakefileMcpConfig) -> list[DoctorFinding]:
...
def _context_findings(
    ...
    for context_name in contexts.names():
        ...
        except (MakefileMcpError, OSError) as exc:
            findings.append(
                DoctorFinding(
...
def _duplicate_context_findings(contexts: Contexts) -> list[DoctorFinding]:
    ...
    for name in contexts.names():
        ...
        except (MakefileMcpError, OSError):
            continue
        ...
        findings.append(
            DoctorFinding(
                ...
                message=(
                    ...
                    "use .makefile-mcp.yaml (governed mode) for explicit agent authorization"
                ),
            )
        )
