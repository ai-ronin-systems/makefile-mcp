"""Stable Makefile MCP data contracts and configuration models."""
...
from makefile_mcp.syntax import (
    BOOLEAN_VALUES,
...
class VariableType(StrEnum):
    ...
    # Make syntax or a recipe argument. It is serialized into MAKEFILE_MCP_INPUT JSON.
    STRING = "string"
...
def _validate_task_variable_mapping(
    ...
    if control:
        raise ValueError(
            "task variable names reserved for Makefile MCP tool controls cannot be exposed: "
            + ", ".join(control)
        )
    reserved = sorted(set(value) & RESERVED_MAKE_VARIABLES)
    if reserved:
        raise ValueError(
            "reserved GNU Make and Makefile MCP variables cannot be exposed: " + ", ".join(reserved)
        )
    return value
...
class DoctorFinding(StrictModel):
    """One diagnostic observation produced by ``makefile-mcp doctor``."""
...
class EnvironmentConfig(StrictModel):
    ...
    def unique_inherit(cls, value: list[str]) -> list[str]:
        ...
        if reserved:
            raise ValueError(
                "GNU Make and Makefile MCP control variables cannot be inherited: "
                + ", ".join(reserved)
            )
        return list(dict.fromkeys(value))
    ...
    def valid_allowed_names(cls, value: dict[str, str]) -> dict[str, str]:
        ...
        if reserved:
            raise ValueError(
                "GNU Make and Makefile MCP control variables cannot be configured: "
                + ", ".join(reserved)
            )
        return value


class MakefileMcpConfig(StrictModel):
    """Validated contents of one optional ``.makefile-mcp.yaml`` policy file."""
    ...
    @model_validator(mode="after")
    def validate_task_contexts(self) -> "MakefileMcpConfig":
