...
from makefile_mcp.mcp import McpPresentation
...
def _fail(exc: MakefileMcpError) -> Never:
...
def callback(
    ...
    version: Annotated[
        ...
        typer.Option(
            ...
            is_eager=True,
            help="Show the Makefile MCP version and exit.",
        ),
    ] = False,
) -> None:
...
def list_command(
    ...
    except MakefileMcpError as exc:
        _fail(exc)
    if json_output:
        _json(tasks)
        return
...
def serve_command(
    ...
    from makefile_mcp.mcp.server import run_stdio_server
...
def main() -> None:
    """Run the Makefile MCP command-line interface."""
    cli()
