...
from makefile_mcp.catalog import Catalog, Contexts
from makefile_mcp.errors import ExecutionStartError
from makefile_mcp.filesystem import FileContextLock
from makefile_mcp.inputs import make_variables, string_input_file, validate_variables
from makefile_mcp.models import (
    EnvironmentConfig,
    MakefileMcpConfig,
    ProcessResult,
    TaskResult,
...
from makefile_mcp.syntax import MAKEFILE_MCP_INPUT_VARIABLE
...
def build_make_argv(
    ...
    *,
    ...
    if string_input_file is not None:
        ...
        # contents themselves remain in JSON. MAKEFILE_MCP_INPUT is reserved from user config.
        assignments.append(f"{MAKEFILE_MCP_INPUT_VARIABLE}={string_input_file}")
...
class TaskExecutor:
    ...
    async def run(
        ...
        *,
        ...
        # A missing process here would violate the try/except invariant above: every
        # preparation/start failure returns through ``message`` before this point.
