...
# MAKEFILE_MCP_INPUT is owned by the executor. String inputs are serialized to a private JSON
# payload and only this generated pathname is transported through GNU Make.
MAKEFILE_MCP_INPUT_VARIABLE = "MAKEFILE_MCP_INPUT"
...
RESERVED_MAKE_VARIABLES = frozenset(
    {
        ...
        "SHELL",
        "VPATH",
        MAKEFILE_MCP_INPUT_VARIABLE,
    }
)


def is_identifier_name(name: str) -> bool:
    """Return whether *name* uses Makefile MCP's simple identifier grammar."""
...
def parse_boolean_literal(raw: str) -> bool:
    ...
    Args:
        raw: Text accepted by Makefile MCP's boolean grammar.
