...
from makefile_mcp.errors import ConfigurationError, TaskBusyError, UnsafePathError
...
def detect_repository_root(start: Path) -> Path:
    ...
    # `.makefile-mcp.yaml` could outrank a nearer child `.git` repository and bind
    # the process to the wrong project's authorization policy.
    for candidate in candidates:
        if (candidate / ".makefile-mcp.yaml").exists() or (candidate / ".git").exists():
            return candidate
...
def fingerprint(paths: list[Path]) -> Fingerprint:
    ...
    for path in sorted(set(paths), key=str):
        ...
        try:
            target = path.stat()
            target_values: tuple[int, int, int, int] = (
                target.st_mtime_ns,
                target.st_ctime_ns,
                target.st_size,
                target.st_ino,
...
class FileContextLock:
    ...
    def __init__(self, root: Path):
        """Create a lock manager rooted under ``.makefile-mcp/locks``."""
        self._directory = root / ".makefile-mcp" / "locks"
