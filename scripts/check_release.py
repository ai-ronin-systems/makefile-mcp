#!/usr/bin/env python3
"""Validate JMIM release identity and built distribution metadata."""
...
VERSION_FILE = ROOT / "src" / "make_mcp" / "version.py"
...
def validate_distributions(dist: Path, expected_name: str, expected_version: str) -> None:
    ...
    unexpected = sorted(
        path.name for path in dist.iterdir() if path.is_file() and path not in {*wheels, *sdists}
    )
