...
from make_mcp.errors import TaskNotExposed, TaskNotFound
...
def test_phantom_vpath_target_cannot_become_callable_via_implicit_rule(repo):
    ...
    # GNU Make could build `src` through an implicit rule when src.c exists, but JMIM must not
    # authorize it unless an actual supported rule declaration exposed that name.
