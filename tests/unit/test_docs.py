...
CANONICAL_DOCS = {
    ...
    "docs/development.md",
    "docs/releasing.md",
    "docs/integrations/audithound.md",
}
_LINK = re.compile(r"\[[^\]]+\]\(([^)]+)\)")
...
def _slug(heading: str) -> str:
    """Approximate GitHub Markdown anchors for the simple headings used by JMIM docs."""
...
def test_public_markdown_has_no_empty_list_items_or_legacy_product_name():
    problems: list[str] = []
    for path in _markdown_files():
        for number, line in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
            ...
            if "Make MCP" in line:
                problems.append(f"{path.relative_to(PROJECT_ROOT)}:{number}: legacy product name")
    assert problems == []


def test_cli_reference_covers_every_public_cli_command():
    cli_source = (PROJECT_ROOT / "src/make_mcp/cli.py").read_text(encoding="utf-8")
