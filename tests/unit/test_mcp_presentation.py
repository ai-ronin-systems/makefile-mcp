...
from makefile_mcp.mcp.presentation import McpPresentation, build_direct_tools, direct_tool_base_name
from makefile_mcp.models import TaskDefinition
...
def test_direct_tool_name_resolution_survives_short_hash_collision(tmp_path):
    from makefile_mcp.app import build_application

    # These two valid target names deliberately collide on the first eight SHA-256 hex digits
    # used by the readable collision suffix. A third earlier target occupies the unsuffixed
    # slug, forcing both colliding identities through the suffix path.
    first = "a@//+/+///b"
    second = "a@@/@@@@@@+b"
    (tmp_path / "Makefile").write_text(
        f".PHONY: a+b {first} {second}\na+b:\n\t@true\n{first}:\n\t@true\n{second}:\n\t@true\n",
        encoding="utf-8",
    )
    ...
    assert names[1].startswith("make_a_b_37c4ee04")
    assert names[2].startswith("make_a_b_37c4ee04")
    assert names[1] != names[2]


def test_direct_token_and_path_annotations_expose_lexical_patterns(app_for):
    app = app_for(
        ".PHONY: deploy\ndeploy:\n\t@true\n",
        "schema_version: 1\n"
        "tasks:\n"
    ...
    signature = inspect.signature(build_direct_tools(app)[0].fn)

    token_schema = TypeAdapter(signature.parameters["TOKEN"].annotation).json_schema()
    path_schema = TypeAdapter(signature.parameters["PATH_VALUE"].annotation).json_schema()

    assert token_schema["pattern"] == r"^[A-Za-z0-9][A-Za-z0-9._/@:+-]*$"
    assert path_schema["pattern"] == r"^[A-Za-z0-9._/@:+/-]+$"
...
def test_runtime_task_contract_rejects_python_keyword_variable_names():
    ...
    from makefile_mcp.models import VariableSpec
...
def test_task_response_raises_for_non_passing_execution():
    ...
    from makefile_mcp.mcp.presentation import task_response
    from makefile_mcp.models import TaskResult, TaskStatus
...
def test_task_response_bounds_entire_error_including_task_and_context_identity():
    from datetime import UTC, datetime
    ...
    assert message.endswith("[MCP diagnostic truncated]")
