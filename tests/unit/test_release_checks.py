...
def _write_fake_distributions(dist: Path, version: str = "0.1.0") -> None:
    ...
    wheel = dist / f"make_mcp-{version}-py3-none-any.whl"
    with zipfile.ZipFile(wheel, mode="w") as archive:
        archive.writestr(f"make_mcp-{version}.dist-info/METADATA", metadata)

    sdist = dist / f"make_mcp-{version}.tar.gz"
    payload = metadata.encode("utf-8")
    with tarfile.open(sdist, mode="w:gz") as archive:
        info = tarfile.TarInfo(name=f"make_mcp-{version}/PKG-INFO")
...
def test_release_check_rejects_mismatched_distribution_metadata(tmp_path: Path):
    ...
    assert result.returncode == 2
    assert "expected make-mcp 0.1.0" in result.stdout
