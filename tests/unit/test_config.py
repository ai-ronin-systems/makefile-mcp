...
from makefile_mcp.config import load_config
...
def test_config_rejects_make_control_variables_in_environment(repo):
    for section in (
        ...
        "allow:\n    MAKEFLAGS: --eval=bad",
        "allow:\n    MAKEFILE_MCP_INPUT: /tmp/attacker.json",
    ):
        root = repo(
            "test:\n\t@true\n",
            "schema_version: 1\nenvironment:\n  " + section + "\ntasks:\n  test: {}\n",
...
def test_config_rejects_preview_as_task_variable_name(repo):
    ...
    with pytest.raises(ConfigurationError, match="reserved for Makefile MCP tool controls"):
        load_config(root)
