...
In Just Make It MCP (JMIM), `.make-mcp.yaml` is optional. Its **presence** enables governed mode; its contents define the governed execution contract.
...
Variable names must be ordinary identifiers, must not be Python keywords such as `class` or `async`, and cannot use reserved GNU Make/JMIM control names. `preview` is also reserved because it is the cross-presentation JMIM execution-preview control. The Python-keyword restriction keeps one governed contract valid in both generic and direct MCP presentations. A variable cannot be both `required: true` and have a `default`; those contracts are contradictory and are rejected at configuration load.
...
`MAKE_MCP_INPUT` when arbitrary path-like text is required.

### `string`

Accepts arbitrary text, including whitespace and newlines. String values do not use direct GNU Make variable transport; they are serialized into the private per-invocation JSON payload referenced by `MAKE_MCP_INPUT`.
...
JMIM constructs a **fresh filtered child environment** for GNU Make; it does not inherit the full parent process environment.
...
Using only `inherit: [API_TOKEN]` intentionally removes `PATH`, `HOME`, and `USER` from the inherited set. If no effective `PATH` is supplied, process execution falls back to the platform default executable-search path. `make-mcp doctor` checks GNU Make availability against this same effective `PATH`.

GNU Make/JMIM control variables are rejected. Environment configuration is trusted operator policy; see [Security](security.md#make-variables-and-process-environments).

## Exposure and presentation are not configuration keys

Exposure mode is selected only by `.make-mcp.yaml` presence:
...
Configuration presence and contents are loaded once per application/server lifetime. If `.make-mcp.yaml` is added, removed, or changed while JMIM is running, subsequent operations fail closed and require restart; policy is never hot-reloaded mid-process.
