...
Makefile MCP is intentionally small: one callable catalog, one execution path, and thin CLI/MCP presentations.
...
---------------                  ----------------
no .makefile-mcp.yaml -> auto        direct | generic | both
.makefile-mcp.yaml    -> governed
```
...
├── config.py       # optional .makefile-mcp.yaml loading + exposure-mode decision
...
When `.makefile-mcp.yaml` is absent, every conservatively discovered root target becomes callable. Tasks are parameterless and use default limits/risk metadata.

### Governed

When `.makefile-mcp.yaml` exists, exposure is deny-by-default. A target must both exist in conservative discovery and be explicitly enabled for the selected context.
...
arbitrary string
  ...
  -> generated MAKEFILE_MCP_INPUT path
  -> make -f Makefile target
```
...
`.makefile-mcp.yaml` presence and contents are resolved in one configuration-load operation per `Application` lifetime. The startup fingerprint is checked on subsequent operations; any policy-file appearance, removal, or modification fails closed and requires restart rather than being hot-reloaded.
...
7. **Policy is startup-stable.** `.makefile-mcp.yaml` changes fail closed and require restart rather than being hot-reloaded.
