...
Just Make It MCP (JMIM) exposes one authorized application/catalog/executor through three MCP presentation modes:
...
```text
.make-mcp.yaml absent/present  -> auto/governed authorization
...
JMIM snapshots the callable catalog at server startup and registers one MCP tool per authorized `(context, target)` pair.
...
Generated names use MCP-safe characters, are capped at 128 characters, and receive a stable short hash only when truncation or collision resolution requires it. The generated name is presentation metadata; authorization and execution retain the original context/target identity.
...
JMIM does **not** encode execution failures as successful `{ "ok": false }` payloads.
...
MCP cancellation is control flow, not a completed task result. JMIM terminates the normal task process group, completes bounded pipe cleanup, and then re-raises cancellation so the MCP request unwinds normally.

A deliberately detached new session is outside JMIM's documented containment boundary; tasks are expected to be foreground/bounded jobs.

## Progress and completion feedback

Execution tools receive an SDK-injected MCP `Context` that is not exposed in the model-visible input schema. When the client requests progress, JMIM reports truthful lifecycle events:
...
JMIM does not invent intermediate percentages because GNU Make does not provide generic task-progress information. Progress remains an MCP-adapter concern and never enters `Application`, `TaskExecutor`, or process management.

## Preview

Both presentations expose the same JMIM-owned boolean control:
...
Catalog/description operations are marked read-only and closed-world. Make execution tools use deliberately conservative annotations: they are not declared read-only or idempotent and are treated as potentially destructive/open-world regardless of JMIM's advisory `risk` metadata.
