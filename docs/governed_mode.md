...
In Makefile MCP, governed mode turns a repository's Makefile surface into an explicit agent execution contract.
...
```text
.makefile-mcp.yaml absent  -> auto mode
.makefile-mcp.yaml present -> governed mode
```

There is no `mode:` setting. Creating `.makefile-mcp.yaml` immediately changes exposure to deny-by-default. An empty file therefore exposes no tasks.

Configuration is loaded once for an application/server lifetime. If `.makefile-mcp.yaml` is created, deleted, or edited while Makefile MCP is running, subsequent operations fail closed until the process is restarted. Authorization policy is never hot-reloaded mid-process.
...
```bash
makefile-mcp list --context backend
makefile-mcp run test --context backend
```
...
```bash
makefile-mcp run deploy ENV=staging WORKERS=4 RELEASE_NOTES='release candidate'
makefile-mcp run deploy ENV=staging --preview
```
...
Risk is advisory Makefile MCP metadata for clients. It is not authorization, does not imply read-only/idempotent/closed-world MCP behavior, and Makefile MCP does not infer it from target names. Omitted risk is `unknown`. Execution tools therefore use conservative MCP behavioral annotations independently of this field.
...
Start with auto mode for a trusted developer repository whose discovered Make surface is acceptable. Add `.makefile-mcp.yaml` when you need one or more of:
...
Do not duplicate recipes or workflow logic in `.makefile-mcp.yaml`. Keep commands, dependencies, ordering, and implementation in Make.
