...
Just Make It MCP (JMIM) is a standard local stdio MCP server. Client integrations launch the existing `make-mcp` CLI; JMIM does not contain Codex-, Claude-, LangChain-, Cursor-, or VS Code-specific execution code.
...
> **Stability boundary:** the `make-mcp ... serve` command line is JMIM's contract. Codex, Claude Code, LangChain, Cursor, and VS Code own their respective configuration formats and may change them independently. Treat the snippets below as integration examples and verify client-specific syntax against the corresponding client documentation when upgrading that client.
...
For governed agent access, create `.make-mcp.yaml` before connecting the client. Auto mode intentionally exposes every conservatively discovered root target and is intended only for trusted local use.
...
Add JMIM as a stdio server:

```bash
claude mcp add jmim -- make-mcp --root /absolute/path/to/repo serve --tools direct
```
...
Install LangChain's MCP adapter package in the orchestrator environment, then point it at the normal JMIM CLI:
...
A persistent `pipx install make-mcp` or `uv tool install make-mcp` avoids startup package-resolution work and is preferable for regular use.
...
```bash
make-mcp --root /absolute/path/to/repo doctor
make-mcp --root /absolute/path/to/repo list
```

If `.make-mcp.yaml` or the direct-tool inventory changes, restart the MCP server. Policy changes deliberately fail closed until restart.
