...
Makefile MCP builds its callable inventory with a deliberately conservative **static** inspector. Discovery answers one security-sensitive question:

> Which target names can Makefile MCP safely recognize without evaluating GNU Make semantics?
...
## Why Makefile MCP does not use `make -qp`
...
Makefile MCP therefore does not execute GNU Make merely to enumerate MCP tools. Discovery trades completeness for a non-evaluating authorization inventory. GNU Make remains authoritative only when an already-authorized task is actually invoked.
...
Execution later pins the same top-level file with `make -f`. GNU Make may still load dynamic or external includes at execution time; those files are trusted execution code but may be outside Makefile MCP's discoverable static inventory.
Makefile MCP does not disable GNU Make's normal built-in variables or implicit-rule semantics; once a target name is authorized, GNU Make remains the execution engine for that target.
...
Dynamic, variable-expanded, globbed, escaped, or otherwise unsupported include expressions are not interpreted. External/out-of-repository includes may affect final GNU Make execution but their targets are not guaranteed to be discoverable by Makefile MCP.
...
A valid GNU Make target can be absent from Makefile MCP's inventory when the target is generated or expressed through syntax the static inspector intentionally does not model. The recommended remediation is to provide a small, stable wrapper target with ordinary syntax:

```make
.PHONY: agent-verify
agent-verify: ## Stable Makefile MCP entry point
...
Makefile MCP does not sandbox trusted Makefiles or filesystem reads performed by GNU Make. See [Security](security.md) for the complete trust boundary.
...
`.makefile-mcp.yaml` is a separate startup authorization policy. It is never hot-reloaded; if its presence or contents change, subsequent operations fail closed until Makefile MCP restarts.
