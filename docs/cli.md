...
`makefile-mcp` is the stable command-line entry point for Makefile MCP. The CLI is a thin adapter over the same `Application`, catalog, authorization, validation, and executor used by MCP.

## Global options

```text
makefile-mcp [--root PATH] [--version] COMMAND ...
```

| Option | Meaning |
| --- | --- |
| `--root PATH` | Start repository-root detection from `PATH`; defaults to the current directory. |
| `--version` | Print the Makefile MCP package version and exit. |

`--root` is a **root-detection starting path**, not a forced Makefile directory. Makefile MCP searches upward for the nearest `.makefile-mcp.yaml` or `.git` boundary; only when neither exists does it fall back to the nearest conventional `Makefile`. This prevents a distant parent policy from binding a nearer repository boundary.

## `list`

```text
makefile-mcp list [--context NAME] [--json]
```
...
```text
makefile-mcp describe TASK [--context NAME] [--json]
```
...
```text
makefile-mcp run TASK [KEY=VALUE ...] [--context NAME] [--preview] [--json]
```
...
```text
makefile-mcp serve [--tools direct|generic|both]
```

Runs the stdio MCP server. `direct` is the default. No HTTP listener, TLS layer, or network authentication service is created by Makefile MCP.
...
| `2` | Invalid CLI usage or an expected Makefile MCP domain/configuration error prevented the operation. |
...
Makefile MCP searches upward for the nearest repository/config boundary.
